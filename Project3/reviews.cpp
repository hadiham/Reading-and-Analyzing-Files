...
double reviewScore(const vector<string> &reviewWords, const vector<string> &keywords, const vector<double> &weights) {
    // TODO: Write an implementation for this function!
    vector<string> review_Words(reviewWords);
     preprocessReview(review_Words);

     double totalScore = 0;
     for(size_t i=0; i < review_Words.size(); ++i) {
        totalScore += wordWeight(review_Words[i], keywords, weights);
      }
     return totalScore;
}
